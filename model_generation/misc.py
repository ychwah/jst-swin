...
def save_img(img, output_name="output_img.png"):
    if img.shape[-1] == 3:
        Image.fromarray((clip_image(img) * 255).astype('uint8'), 'RGB').save(output_name)
    else:
        Image.fromarray((clip_image(img) * 255).astype('uint8')).save(output_name, optimize=True, compress_level=0)
